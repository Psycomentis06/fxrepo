...
import {ImagePostDetailsModel, ImagePostListModel, ImagePostModel} from "../models/image-post";
...
export class ImagePostService {
  ...
  readonly GET_IMAGE_POST_BY_SLUG_ENDPOINT = environment.api.fx_main.v1 + '/post/image/slug/{slug}';
  ...
  getImagePostBySlug(slug: string) {
    return this.http.get<ResponseModel<ImagePostDetailsModel>>(this.GET_IMAGE_POST_BY_SLUG_ENDPOINT.replace("{slug}", slug))
  }
}
